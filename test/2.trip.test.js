...
describe("Acme-explorer trips tests", () => {
  ...
  it("Put a not published Trip -v2", (done) => {
    ...
    idtoken_collector.getIdToken(manager.email).then((idtoken) => {
      chai
        ...
        .set("idtoken", idtoken)
        .end((err, res) => {
          expect(res.body.title).to.equal(title_test);
          expect(res).to.have.status(200);
          done();
        });
    });
  ...
  it("Publish a not published Trip -v2", (done) => {
    idtoken_collector.getIdToken(manager.email).then((idtoken) => {
      chai
        .request(app)
        .put("/v2/trips/" + trip_not_published.id + "/publish")
        .set("idtoken", idtoken)
        .end((err, res) => {
          expect(res).to.have.status(200);
          done();
  ...
  it("Delete a not published Trip -v2", (done) => {
    idtoken_collector.getIdToken(manager.email).then((idtoken) => {
      chai
        .request(app)
        .delete("/v2/trips/" + trip_not_published.id)
        .set("idtoken", idtoken)
        .end((err, res) => {
          expect(res).to.have.status(200);
          done();
  ...
  it("Delete a published Trip -v2", (done) => {
    idtoken_collector.getIdToken(manager.email).then((idtoken) => {
      chai
        .request(app)
        .delete("/v2/trips/" + trip_published.id)
        .set("idtoken", idtoken)
        .end((err, res) => {
          expect(res).to.have.status(405);
          done();
  ...
  it("Cancel a published Trip that don't have any application -v2", (done) => {
    idtoken_collector.getIdToken(manager.email).then((idtoken) => {
      chai
        .request(app)
        .put("/v2/trips/" + trip_published.id + "/cancel")
        .set("idtoken", idtoken)
